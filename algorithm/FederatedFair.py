...
def D_hat_θ(client_dataset, mask_s1_flag, client_model, criterion, device, hypothesis):
    ...
    X_sa_c0, X_sa_c1, y_sa_c0, y_sa_c1 = torch.tensor(np.array(X_sa_c0)).to(device), torch.tensor(np.array(X_sa_c1)).to(device),\
                                         torch.tensor(np.array(y_sa_c0)).to(device), torch.tensor(np.array(y_sa_c1)).to(device)
    ...
    X_sb_c0, X_sb_c1, y_sb_c0, y_sb_c1 = torch.tensor(np.array(X_sb_c0)).to(device), torch.tensor(np.array(X_sb_c1)).to(device),\
                                         torch.tensor(np.array(y_sb_c0)).to(device), torch.tensor(np.array(y_sb_c1)).to(device)
...
def Fed_Fair_LR(device,
    ...
    for iter_t in range(algorithm_epoch_T):
        ...
        # L_θ_λ_a_λ_b += λ_a * ((client_D_hat_list / num_clients_m).sum() - ϵ)  # 2nd term in Eq.8
        # L_θ_λ_a_λ_b += λ_b * ((client_D_hat_list / num_clients_m).sum() + ϵ)  # 3rd term in Eq.8
        #
        # # Equation 9 in paper
