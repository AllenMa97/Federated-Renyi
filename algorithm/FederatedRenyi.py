...
def r_hat_p_initialization(num_clients_m, mask_s1_flag, training_dataset, client_dataset_list, p):
    r_bar_k_p_list = []
    for k in range(num_clients_m):
        if mask_s1_flag:
...
def argmax_v_LR(idxs_users, local_model_list, mask_s1_flag, training_dataset, client_dataset_list, r_hat_p0, r_hat_p1):
    j_bar_c0_p0_list = []
    j_bar_c0_p1_list = []
    j_bar_c1_p0_list = []
    j_bar_c1_p1_list = []
    ...
    for id in idxs_users:
        ...
        j_bar_c0_p0 = (P_joint_c0_p0 / P_s_p0).to(device)
        j_bar_c0_p1 = (P_joint_c0_p1 / P_s_p1).to(device)
        j_bar_c1_p0 = (P_joint_c1_p0 / P_s_p0).to(device)
        j_bar_c1_p1 = (P_joint_c1_p1 / P_s_p1).to(device)
        ...
        u_bar_c0 = (sum(y_hat_θ_c0) / len(y_hat_θ)).to(device)
        u_bar_c1 = (sum(y_hat_θ_c1) / len(y_hat_θ)).to(device)  # u^bar_k(c)
        u_bar_c0_list.append(u_bar_c0)
        u_bar_c1_list.append(u_bar_c1)

    j_hat_c0_p0 = torch.tensor(j_bar_c0_p0_list).mean()
    j_hat_c0_p1 = torch.tensor(j_bar_c0_p1_list).mean()
    j_hat_c1_p0 = torch.tensor(j_bar_c1_p0_list).mean()
    j_hat_c1_p1 = torch.tensor(j_bar_c1_p1_list).mean()
    u_hat_c0 = torch.tensor(u_bar_c0_list).mean()
    u_hat_c1 = torch.tensor(u_bar_c1_list).mean()
...
def argmax_v_NN(idxs_users, local_model_list, mask_s1_flag, training_dataset, client_dataset_list, r_hat_p0, r_hat_p1):
    j_bar_c0_p0_list = []
    j_bar_c0_p1_list = []
    j_bar_c1_p0_list = []
    j_bar_c1_p1_list = []
...
def Fed_Renyi_LR(device,
    ...
    # global_v_0 = global_v_0_initialization(num_clients_m, mask_s1_flag, training_dataset, client_dataset_list)
    global_v = torch.rand(2, 1)

    r_hat_p0 = r_hat_p_initialization(num_clients_m, mask_s1_flag, training_dataset, client_dataset_list, p=0)
    r_hat_p1 = r_hat_p_initialization(num_clients_m, mask_s1_flag, training_dataset, client_dataset_list, p=1)
    ...
    for iter_t in range(algorithm_epoch_T):
        ...
        if (iter_t + 1) % communication_round_I == 0:
            ...
            backup_v = global_v
            try:
                global_v = argmax_v_LR(idxs_users, local_model_list, mask_s1_flag, training_dataset, client_dataset_list, r_hat_p0, r_hat_p1)
            except Exception:
                global_v = backup_v
...
def Fed_Renyi_NN(device,
    ...
    for iter_t in range(algorithm_epoch_T):
        ...
        if (iter_t + 1) % communication_round_I == 0:
            ...
            logger.info("********** Global v update **********")
            global_v = argmax_v_NN(idxs_users, local_model_list, mask_s1_flag, training_dataset, client_dataset_list, r_hat_p0, r_hat_p1)
